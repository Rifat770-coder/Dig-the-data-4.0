...
export async function updateAuthMode(mode: AuthMode, updatedBy?: string): Promise<void> {
  try {
    ...
    try {
      ...
      if (isAppwriteError(updateError) && (updateError as AppwriteError).code === 404) {
    ...
    if (isAppwriteError(error)) {
      const appwriteErr = error as AppwriteError;
      // If Appwrite is not available or collection doesn't exist, use localStorage as fallback
      if (appwriteErr.code === 404 || appwriteErr.message?.includes('Collection') || appwriteErr.message?.includes('Database')) {
      ...
      if (appwriteErr.code === 401 || appwriteErr.code === 403) {
        throw new AuthPermissionError('Insufficient permissions to update authentication mode');
      }
      
      if (typeof appwriteErr.code === 'number' && appwriteErr.code >= 500) {
