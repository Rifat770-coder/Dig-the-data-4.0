...
export default function IndoorMissionPage() {
  ...
  const syncScoreToAppwrite = async (teamCodeParam: string, score: number) => {
    try {
      // This will set the team's score to the exact value (not add to it)
      // We need to get the team first and update it
      const { getTeamByCode, updateOwnTeamScore } = await import('@/lib/team-api');
      const team = await getTeamByCode(teamCodeParam);
      
      if (team && team.$id) {
        await updateOwnTeamScore(team.$id, score);
        console.log(`Synced ${score} points to team ${teamCodeParam}`);
      }
    } catch (error) {
